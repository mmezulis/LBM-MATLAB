...
Lx = 500; Nx = Lx; dx = Lx/Nx; X = linspace(dx/2, Lx-dx/2, Nx); %Režģa izmēri
Ly = 500; Ny = Ly + 2; Y = linspace(-dx/2, Ly+dx/2, Ny);
[XX, YY] = meshgrid(X, Y);
endT = 10000; dt = 1; tt = 0:dt:endT; tau = 1;

% Šķērslis - cilinds apgabala centrā
isObstacle = gpuArray((((XX' - Lx/2).^2 + (YY' - Ly/2).^2) < (Lx*0.2)^2) | (YY' < 0) | (YY' > Ly));

% Sākumnosacījumi
RHO = gpuArray.ones(Nx, Ny);
UX = gpuArray.zeros(Nx, Ny);
UY = gpuArray.zeros(Nx, Ny);
...
function f = streaming(ftemp, fEq)
    ...
    for ie=1:b
        % Vērtības, kuras iegāja iekšā sienās
...
function [RHO, UX, UY, V] = calculate_macro(f)
    ...
    UX = gpuArray.zeros([size(f,1), size(f,2)]);
    UY = gpuArray.zeros([size(f,1), size(f,2)]);
    for ie=1:b
       UX(:, :) = UX(:, :) + f(:, :, ie)*c(ie, 1);
       UY(:, :) = UY(:, :) + f(:, :, ie)*c(ie, 2);
    end
...
function ftemp = collision(f, fEq)
    global Nx Ny b stepProp stepPropInv;

    ftemp = gpuArray.zeros(Nx, Ny, b);
    for ie = 1:b
